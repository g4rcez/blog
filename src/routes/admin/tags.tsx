...
export const loader: Remix.LoaderFunction<Props> = async () => {
    ...
    <li className="my-4">
      <fetcher.Form method={Http.Post} ref={form}>
        <fieldset className={`flex flex-nowrap gap-x-4 justify-between items-end ${isSubmitting ? "opacity-25" : ""}`} disabled={isSubmitting}>
          <Input<Tag> placeholder="Label" name="label" className="w-full" required autoFocus />
...
const TagInline: React.VFC<{ tag: Tag | null }> = ({ tag }) => {
  const fetcher = useFetcher();
  const isSubmitting = fetcher.state === "loading" || fetcher.state === "submitting";
  return (
    <li className="my-4">
      <fetcher.Form method={Http.Put}>
        <fieldset className={`flex flex-nowrap gap-x-4 justify-between items-end ${isSubmitting ? "opacity-25" : ""}`} disabled={isSubmitting}>
          ...
          <Input<Tag> placeholder="Label" name="label" defaultValue={tag?.label} className="w-full" required />
    ...
    <Container>
      ...
      <ul className="list-none list-inside">
        ...
        {loaderData.tags.map((tag) => (
          <TagInline key={tag.tagId} tag={tag} />
        ))}
      </ul>
    </Container>
  );

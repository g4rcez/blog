...
const className =
  "flex items-center justify-center w-fit font-extrabold text-xl";
...
const NewTagInline: React.FC<{ autoFocus: boolean }> = ({ autoFocus }) => {
  ...
  const isAdding =
    ...
    fetcher.submission.formData.get(Remix.FormActionKey) === "create";
  ...
  return (
    <li className="my-4">
      <fetcher.Form method={Http.Post} ref={form}>
        <fieldset
          className={`flex flex-nowrap gap-x-4 justify-between items-end ${
...
const TagInline: React.FC<{ tag: Tag | null; autoFocus: boolean }> = ({
  tag,
  autoFocus,
}) => {
  const fetcher = useFetcher();
        ...
        {loaderData.tags.length > 0 &&
          loaderData.tags.map((tag) => (
            <TagInline
              key={tag.tagId}
              tag={tag}
              autoFocus={tag.label === loaderData.tagQueryString}
            />
          ))}
      </ul>
    </Container>

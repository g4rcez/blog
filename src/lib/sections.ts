...
const isHxNode = <N extends number>(n: N,node: Node): node is HeadingNode & {
  attributes: {
    level: N
  }
} => isHeadingNode(node) && node.attributes.level === n

function isH2Node(node: Node) {
  return isHxNode(2, node)
}

function isH3Node(node: Node) {
  return isHxNode(3, node)
}

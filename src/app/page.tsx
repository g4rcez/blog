      ...
      >
        <defs>
          <pattern id={id} width="6" height="8" patternUnits="userSpaceOnUse">
            <path
              d="M0 0H6M0 8H6"
              className="dark:stroke-white/10 xl:stroke-black/10"
              fill="none"
            />
          </pattern>
        </defs>
  ...
  <div className="relative flex-none overflow-hidden px-6 lg:pointer-events-none lg:fixed lg:inset-0 lg:z-40 lg:flex lg:px-0">
    ...
    <div className="relative flex w-full lg:pointer-events-auto lg:mr-[calc(max(2rem,50%-38rem)+40rem)] lg:min-w-[45rem] lg:overflow-y-auto lg:overflow-x-hidden lg:pl-[max(4rem,calc(50%-38rem))]">
      <div className="mx-auto max-w-lg lg:mx-0 lg:flex lg:w-96 lg:max-w-none lg:flex-col lg:before:flex-1 lg:before:pt-6">
        <main className="pb-16 pt-20 sm:pb-20 sm:pt-32 lg:py-20">
          {props.children}
        </main>
...
const Tags = ({ tags, id }: { tags: string[]; id: string }) => (
  <ul className="flex flex-row gap-4 text-xs my-2 flex-wrap">
    {tags.map((x) => (
      <li
        key={`${id}-${x}-tag`}
        className="px-4 py-1 bg-indigo-900 text-white rounded-lg"
      >
...
function Intro({ tags }: { tags: string[] }) {
  return (
    <Fragment>
      ...
      <h1 className="mt-6 font-display text-4xl/tight font-medium dark:text-white">
        Bem vindo ao meu blog
      </h1>
      <p className="mt-4 text-md/6 dark:text-zinc-200">
        Aqui você vai encontrar conteúdo sobre frontend em geral, focando em
        Typescript, React, Tailwindcss, NextJS e mais da sopa de letrinhas do
        universo frontend
      </p>
      <div className="flex flex-col">
        <form className="border text-sm dark:border-slate-600 border-slate-300 flex flex-row items-center flex-nowrap text-white w-full rounded-lg my-4">
          ...
          />
          <button className="p-1.5 px-4 bg-indigo-800 mx-1 rounded-lg flex text-sm">
            Buscar
          </button>
        </form>
      ...
      <nav className="mt-8 flex flex-wrap justify-center gap-x-1 gap-y-3 sm:gap-x-2 lg:justify-start">
        ...
        <IconLink
          href="https://twitter.com/garcez_allan"

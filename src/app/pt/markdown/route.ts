...
export function GET() {
    const posts = getPosts("pt-BR");
    const body = [
        `# ${homeCopy["pt-BR"].title}`,
        "",
        homeCopy["pt-BR"].description,
        "",
        `Autor: ${BlogConfig.user.name}`,
        `Canônico: ${SITE_URL}/pt`,
        "",
        "## Posts recentes",
        "",
        ...posts
            .slice(0, 20)
            .map((post) => `- [${post.info.title}](${SITE_URL}/pt${post.href}) — ${post.info.description}`),
        "",
        "## Recursos para agentes",
        "",
        `- [Product UI Engineer](${SITE_URL}/agents/product-ui-engineer)`,
        `- [Índice de agent skills](${SITE_URL}/.well-known/agent-skills/index.json)`,

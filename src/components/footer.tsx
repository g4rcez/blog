...
const iconClassName = "inline-block w-4 h-4 mr-2";
const linkClassName = "bg-transparent  inline-flex items-center m-2 text-center p-2 hover:underline";
const spanClassName = "text-default";

type SocialMediaRecord = Record<keyof typeof Config, (props: { user: string }) => any>;

const SocialMedia: Partial<SocialMediaRecord> = {
  facebook: ({ user }) => (
    <a
    ...
    >
      ...
      <span className={spanClassName}>Facebook</span>
    </a>
  ),
  github: ({ user }) => (
    <a
  ...
  medium: ({ user }) => (
    ...
    >
      ...
      <span className={spanClassName}>Medium</span>
    </a>
  )
};
export const Footer = () => {
  const components = useMemo(
    () =>
      Object.entries(Config).reduce((acc, x) => {
        const [socialMedia, user] = x;
        const Media = (SocialMedia as any)[socialMedia];
        if (isEmpty(user) || isNil(Media)) {
          return acc;
        }
        return [...acc, <Media key={`${user}-${socialMedia}`} user={user} />];
      }, [] as any[]),
    []
  );
  return (
    <header className="justify-center flex w-full max-w-full mt-8">
      <nav className="flex items-center justify-center flex-wrap p-1 container">
        <div className="w-full items-center flex justify-center flex-wrap">{components}</div>
      </nav>
    </header>
  );
};
